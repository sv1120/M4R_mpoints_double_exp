...
class HybridHawkesExp:
    ...
    def __init__(self, number_of_event_types, number_of_states, events_labels, states_labels):
        ...
        self.impact_decay_ratios = np.zeros(
            (number_of_event_types, number_of_states, number_of_event_types))
        self.impact_coefficients2 = np.zeros(
            (number_of_event_types, number_of_states, number_of_event_types))
        self.decay_coefficients2 = np.zeros(
            (number_of_event_types, number_of_states, number_of_event_types))
        self.impact_decay_ratios2 = np.zeros(
            (number_of_event_types, number_of_states, number_of_event_types))
    ...
    def set_hawkes_parameters(self, base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2):
        r"""
        ...
        if np.shape(impact_coefficients2) != (self.number_of_event_types, self.number_of_states,
                                             self.number_of_event_types):
            raise ValueError('given impact coefficients2 have incorrect shape')
        if np.shape(decay_coefficients2) != (self.number_of_event_types, self.number_of_states,
                                            self.number_of_event_types):
            raise ValueError('given decay coefficients2 have incorrect shape')
        ...
        self.impact_coefficients2 = copy.copy(impact_coefficients2)
        self.decay_coefficients2 = copy.copy(decay_coefficients2)
        self.impact_decay_ratios2 = np.divide(
            impact_coefficients2, decay_coefficients2)

    @staticmethod
    def kernel_at_time(time, alpha, beta, alpha2, beta2):
        r"""
        ...
        """
        return alpha * np.exp(- np.multiply(time, beta))+alpha2 * np.exp(- np.multiply(time, beta2)) # this function is fine
    ...
    def estimate_hawkes_parameters(self, times, events, states, time_start, time_end, maximum_number_of_iterations=2000,
                                   ...
                                   min_decay_coefficient=0.5, max_decay_coefficient=100, parallel_estimation=True, jitter_std=0.1, 
                                   stochastic_init_base=True):
        r"""
        ...
        if number_of_random_guesses > 0:
            ...
            for n in range(number_of_random_guesses):
                ...
                for e in range(self.number_of_event_types):
                    guess_base_rates[e] = average_intensities[e] / 2 # assuming half of events are occurring anyway, and half are triggered by other events
                    # Perturb the initial guess with random noise - stochastic initialiser of base rate
                    if stochastic_init_base == True:
                        guess_base_rates[e] += np.random.normal(scale=jitter_std) # stochastic initialiser, optional for the base rates
                'Decay coefficients'
                guess_decay_coefficients = np.zeros((self.number_of_event_types, self.number_of_states,
                                                     self.number_of_event_types))
                guess_decay_coefficients2 = np.zeros((self.number_of_event_types, self.number_of_states,
                                                     self.number_of_event_types))
                for e1 in range(self.number_of_event_types):
                    for x in range(self.number_of_states):
                        for e2 in range(self.number_of_event_types):
                            ...
                            guess_decay_coefficients2[e1, x, e2] = beta2
                'Impact coefficients'
                guess_impact_coefficients = np.zeros((self.number_of_event_types, self.number_of_states,
                                                      self.number_of_event_types))
                guess_impact_coefficients2 = np.zeros((self.number_of_event_types, self.number_of_states,
                                                      self.number_of_event_types))
                for e1 in range(self.number_of_event_types):
                    for x in range(self.number_of_states):
                        for e2 in range(self.number_of_event_types):
                            ...
                            alpha2 = u2 * guess_decay_coefficients2[e1, x, e2]
                            guess_impact_coefficients[e1, x, e2] = alpha
                            guess_impact_coefficients2[e1, x, e2] = alpha2
                ...
                g = HybridHawkesExp.parameters_to_array(guess_base_rates, guess_impact_coefficients,
                                                        guess_decay_coefficients, guess_impact_coefficients2, guess_decay_coefficients2)
                guesses.append(g)
        ...
        if not parallel_estimation:
            optimal_results = []
            for g in guesses:
                # dimension update from 2 to 4 to include both sets of impact and decay coefficients
                dimension = self.number_of_event_types + 4 * \
            ...
            dimension = 1 + 4 * self.number_of_states * self.number_of_event_types
            bounds = [(parameters_lower_bound,
                       parameters_upper_bound)] * dimension
            ...
            for e in range(self.number_of_event_types):
                ...
                optimal_results = []
                for g in guesses:
                    guess_nus, guess_alphas, guess_betas, guess_alphas2, guess_betas2 = self.array_to_parameters(g, self.number_of_event_types,
                                                                                    self.number_of_states,
                                                                                    self.number_of_event_types)
                ...
                v, a, b, a2, b2= self.array_to_parameters(
                    o.x, self.number_of_event_types, self.number_of_states, 1)
                opt_nus[e:e + 1] = v
                opt_alphas[:, :, e:e + 1] = a
                opt_betas[:, :, e: e + 1] = b

                opt_alphas2[:, :, e:e + 1] = a2
                opt_betas2[:, :, e: e + 1] = b2
                # Save best initial guess
                v, a, b, a2, b2 = self.array_to_parameters(best_initial_guess, self.number_of_event_types,
                                                   self.number_of_states, self.number_of_event_types)
                best_guess_nu[e] = v[e]
                best_guess_alphas[:, :, e] = a[:, :, e]
                best_guess_betas[:, :, e] = b[:, :, e]

                best_guess_alphas2[:, :, e] = a2[:, :, e]
                best_guess_betas2[:, :, e] = b2[:, :, e]
            ...
            best_initial_guess = self.parameters_to_array(
                best_guess_nu, best_guess_alphas, best_guess_betas, best_guess_alphas2, best_guess_betas2)
            o = opt.OptimizeResult()
            x = self.parameters_to_array(opt_nus, opt_alphas, opt_betas, opt_alphas2, opt_betas2)
            o['x'] = x
            o['success'] = success
    ...
    def compute_events_residuals(self, times, events, states, time_start, initial_partial_sums=0, initial_partial_sums2=0):
        r"""
        ...
        s2 = np.zeros((self.number_of_event_types,
                     self.number_of_states, self.number_of_event_types))
        if len(np.shape(initial_partial_sums2)) != 0:
            s2 = initial_partial_sums2
            s2 = np.divide(s2, self.decay_coefficients2)

        return cy.compute_events_residuals(self.base_rates,
                                           self.impact_coefficients,
                                           self.decay_coefficients,
                                           self.impact_coefficients2,
                                           self.decay_coefficients2,
                                           self.number_of_event_types,
                                           self.number_of_states,
                                           times,
                                           events,
    ...
    def compute_total_residuals(self, times, events, states, time_start, initial_partial_sums=0,initial_partial_sums2=0,
                                initial_state=0):
        r"""
    ...
    def simulate(self,
                 ...
                 initial_partial_sums2: Optional[np.array] = None,
                 initial_state: int = 0,
        ...
        if initial_partial_sums2 is None:
            s2 = np.zeros((self.number_of_event_types,
                         self.number_of_states, self.number_of_event_types))
        elif isinstance(initial_partial_sums2, (int, float)):
            s2= initial_partial_sums2 * np.ones((self.number_of_event_types,
                                                self.number_of_states,
                                                self.number_of_event_types), dtype=float)
        else:
            assert isinstance(initial_partial_sums2, np.ndarray)
            assert initial_partial_sums2.shape == (
                self.number_of_event_types, self.number_of_states, self.number_of_event_types)
            s2 = np.array(initial_partial_sums2, dtype=float)
    ...
    def log_likelihood_of_events(self, parameters, times, events, states, time_start, time_end):
        ...
        base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2= \
            HybridHawkesExp.array_to_parameters(
                parameters, self.number_of_event_types, self.number_of_states)
        return cy.log_likelihood_of_events(base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2, number_of_event_types,
    ...
    def gradient(self, parameters, times, events, states, time_start, time_end):
        ...
        base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2 = \
            HybridHawkesExp.array_to_parameters(
                parameters, self.number_of_event_types, self.number_of_states)
        g_base_rates, g_impact_coefficients, g_decay_coefficients, g_impact_coefficients2, g_decay_coefficients2 =\
            cy.gradient(base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2, number_of_event_types,
                        number_of_states, times, events, states, float(time_start), float(time_end))
        return self.parameters_to_array(g_base_rates, g_impact_coefficients, g_decay_coefficients, g_impact_coefficients2, g_decay_coefficients2)
    ...
    def log_likelihood_of_events_partial(self, event_type, parameters, times, events, states, time_start, time_end):
        ...
        number_of_event_types = self.number_of_event_types
        number_of_states = self.number_of_states
        base_rate, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2 = \
            HybridHawkesExp.array_to_parameters(
        ...
        return cy.log_likelihood_of_events_partial(event_type, float(base_rate[0]), impact_coefficients[:, :, 0],
                                                   decay_coefficients[:, :, 0],impact_coefficients2[:, :, 0],
                                                   decay_coefficients2[:, :, 0],
    ...
    def gradient_partial(self, event_type, parameters, times, events, states, time_start, time_end):
        ...
        base_rate, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2 = \
            HybridHawkesExp.array_to_parameters(
                parameters, number_of_event_types, number_of_states, 1)
        g_base_rate, g_impact_coefficients, g_decay_coefficients, g_impact_coefficients2, g_decay_coefficients2 = \
            cy.gradient_partial(event_type, float(base_rate[0]), impact_coefficients[:, :, 0], decay_coefficients[:, :, 0],
                                impact_coefficients2[:, :, 0], decay_coefficients2[:, :, 0],
                                number_of_event_types,
                                number_of_states, times, events, states, float(time_start), float(time_end))
        a = np.zeros((number_of_event_types, number_of_states, 1))
        b = np.zeros((number_of_event_types, number_of_states, 1))
        a2 = np.zeros((number_of_event_types, number_of_states, 1))
        b2 = np.zeros((number_of_event_types, number_of_states, 1))
        a[:, :, 0] = g_impact_coefficients
        b[:, :, 0] = g_decay_coefficients
        a2[:, :, 0] = g_impact_coefficients2
        b2[:, :, 0] = g_decay_coefficients2
        return self.parameters_to_array([g_base_rate], a, b, a2, b2)
    ...
    def intensities_of_events_at_times(self, compute_times, times, events, states):
        ...
        '''Initialise the partial sums S_{e',x',e}, S_{2,e',x',e} that will allow use to compute the intensity recursively'''
        partial_sums = np.zeros(
            (self.number_of_event_types, self.number_of_states, self.number_of_event_types))
        partial_sums2 = np.zeros(
            (self.number_of_event_types, self.number_of_states, self.number_of_event_types))
        for n in range(next_event_time_index):
            ...
            for e in range(self.number_of_event_types):
                ...
                alpha2 = self.impact_coefficients2[event, state, e]
                beta2 = self.decay_coefficients2[event, state, e]
                partial_sums2[event, state, e] += alpha2 * \
                    math.exp(-beta2 * (time_start - time))
        ...
        intensities = self.intensities_of_events(partial_sums, partial_sums2)
        for e in range(self.number_of_event_types):
        ...
        for n in range(1, number_of_times):
                ...
                for e1 in range(self.number_of_event_types):
                    for x in range(self.number_of_states):
                        for e2 in range(self.number_of_event_types):
                            ...
                            beta2 = self.decay_coefficients2[e1, x, e2]
                            partial_sums2[e1, x,
                                         e2] *= math.exp(-beta2 * time_increment)
            ...
            if event >= 0:
                ...
                for e2 in range(self.number_of_event_types):
                    ...
                    alpha2 = self.impact_coefficients2[event, state, e2]
                    partial_sums2[event, state, e2] += alpha2
    ...
    def compute_partial_sums2(self, times, events, states, time_end,
                             initial_partial_sums2=None, time_initial_condition=None):
        r"""
        Computes the partial sums
        ...
        for n in range(index_end):
            ...
            partial_sums2[event, state,
                         :] += np.exp(- self.decay_coefficients2[event, state, :] * (time_end - time))
        partial_sums2 = np.multiply(partial_sums2, self.impact_coefficients2)
        'Add contribution of the given initial condition'
        if np.shape(initial_partial_sums2) != () and time_initial_condition is not None:
            partial_sums2 += np.multiply(np.exp(- self.decay_coefficients2 * (time_end - time_initial_condition)),
                                        initial_partial_sums2)
        return partial_sums2


    def intensity_of_event(self, event_type, partial_sums, partial_sums2):
        r"""
        ...
        :param partial_sums: the partial sums :math:`S_{e',x',e}, S_{2,e',x',e}` at the considered time.
        :rtype: float
        ...
        for e in range(self.number_of_event_types):
            for x in range(self.number_of_states):
                ...
                result += partial_sums2[e, x, event_type]
        return result

    def intensities_of_events(self, partial_sums, partial_sums2):
        r"""
        ...
        :param partial_sums: the partial sums :math:`S_{2,e',x',e}` at the considered time.
        ...
        for e in range(self.number_of_event_types):
            result[e] = self.intensity_of_event(e, partial_sums, partial_sums2)
        return result

    @staticmethod
    def parameters_to_array(base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2):
        r"""
        ...
        result = np.zeros(number_of_event_types_2
        + 4 * number_of_event_types_1
        ...
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    index = number_of_event_types_2 + j * \
                    number_of_event_types_1 * number_of_event_types_2 +\
                    2*number_of_event_types_1*number_of_event_types_2*number_of_states
                    index += i * number_of_event_types_2 + k
                    result[index] = impact_coefficients2[i, j, k]
                    
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    index = number_of_event_types_2
                    index += 3*number_of_event_types_1 * number_of_event_types_2 * number_of_states
                    index += j * number_of_event_types_1 * number_of_event_types_2
                    index += i * number_of_event_types_2 + k
                    result[index] = decay_coefficients[i, j, k]
        ...
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    index = number_of_event_types_2 + j * \
                        number_of_event_types_1 * number_of_event_types_2 +\
                    2*number_of_event_types_2*number_of_event_types_1*number_of_states
                    index += i * number_of_event_types_2 + k
                    impact_coefficients2[i, j, k] = array[index]


        decay_coefficients2 = np.zeros(
            (number_of_event_types_1, number_of_states, number_of_event_types_2))
        for i in range(number_of_event_types_1):
            for j in range(number_of_states):
                for k in range(number_of_event_types_2):
                    ...
                    index += j * number_of_event_types_1 * number_of_event_types_2
                    index += i * number_of_event_types_2 + k
                    decay_coefficients2[i, j, k] = array[index]

    
        return base_rates, impact_coefficients, decay_coefficients, impact_coefficients2, decay_coefficients2
