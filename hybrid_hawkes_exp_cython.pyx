...
def log_likelihood_of_events(np.ndarray[DTYPEf_t, ndim=1] base_rates,
                             ...
                             np.ndarray[DTYPEf_t, ndim=3] impact_coefficients2,
                             np.ndarray[DTYPEf_t, ndim=3] decay_coefficients2,
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 3] partial_sums2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
   
    cdef int n, event, state, e, e1, x, e2, index_end
    cdef double time, previous_time, intensity_of_the_event
    cdef double alpha, beta, ratio, alpha2, beta2, ratio2, time_increment, time_increment_2
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 3] impact_decay_ratios2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
    ...
    for e1 in range(number_of_event_types):
        for x in range(number_of_states):
            for e2 in range(number_of_event_types):
                ...
                alpha2 = impact_coefficients2[e1, x, e2]
                beta2 = decay_coefficients2[e1, x, e2]
                impact_decay_ratios2[e1, x, e2] = alpha2 / beta2
                
    '''Initialise the partial sums S_{e'x'e}, S_{2,e'x'e} that will allow us to compute the intensity recursively;
    and initialise the log-likelihood taking into account the initial condition'''
    cdef double log_likelihood = 0
    cdef double log_likelihood2 = 0
    ...
    for n in range(index_start):
        ...
        for e in range(number_of_event_types):
            ...
            beta2 = decay_coefficients2[event, state, e]
            ratio2 = impact_decay_ratios2[event, state, e]
            partial_sums2[event, state, e] += exp(-beta2 * time_increment)
            log_likelihood -= ratio2 * \
                (exp(-beta2 * time_increment) - exp(-beta2 * time_increment_2))
    ...
    for event in range(number_of_event_types):
        for state in range(number_of_states):
            for e in range(number_of_event_types):
                partial_sums2[event, state, e] = partial_sums2[event,
                                                             state, e] * impact_coefficients2[event, state, e]
    ...
    for n in range(index_start, index_end):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                for e2 in range(number_of_event_types):
                    ...
                    beta2 = decay_coefficients2[e1, x, e2]
                    partial_sums2[e1, x, e2] *= exp(-beta2 * time_increment)
        ...
        for e in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                intensity_of_the_event += partial_sums2[e, x, event]
        ...
        for e in range(number_of_event_types):
            ...
            alpha2 = impact_coefficients2[event, state, e]
            partial_sums2[event, state, e] += alpha2
        previous_time = time
        ...
        for e in range(number_of_event_types):
            ...
            log_likelihood -= ratio2* (1 - exp(-beta2 * time_increment))
...
def log_likelihood_of_events_partial(int event_type,
                                     ...
                                     np.ndarray[DTYPEf_t, ndim=2] impact_coefficients2,
                                     np.ndarray[DTYPEf_t, ndim=2] decay_coefficients2,
                                     int number_of_event_types,
                                     int number_of_states,
                                     np.ndarray[DTYPEf_t, ndim=1] times,
                                     np.ndarray[DTYPEi_t, ndim=1] events,
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 2] partial_sums2 = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf)
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 2] impact_decay_ratios2 = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf)
    ...
    for e1 in range(number_of_event_types):
        for x in range(number_of_states):
            ...
            alpha2 = impact_coefficients2[e1, x]
            beta2 = decay_coefficients2[e1, x]
            impact_decay_ratios2[e1, x] = alpha2 / beta2

    
    '''Initialise the partial sums S_{e'x'}, S_{2,e'x'} that will allow us to compute the intensity recursively;
    and initialise the log-likelihood taking into account the initial condition'''
    cdef double log_likelihood = 0
    ...
    for n in range(index_start):
        ...
        beta2 = decay_coefficients2[event, state]
        ratio2 = impact_decay_ratios2[event, state]
        partial_sums2[event, state] += exp(-beta2 * time_increment)
        log_likelihood -= ratio2 * \
            (exp(-beta2 * time_increment) - exp(-beta2 * time_increment_2))
    ...
    for event in range(number_of_event_types):
        for state in range(number_of_states):
            ...
            partial_sums2[event, state] = partial_sums2[event,
                                                      state] * impact_coefficients2[event, state]
    ...
    for n in range(index_start, index_end):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                beta2 = decay_coefficients[e1, x]
                partial_sums2[e1, x] *= exp(-beta2 * time_increment)
        ...
        if event == event_type:
            ...
            for e in range(number_of_event_types):
                for x in range(number_of_states):
                    ...
                    intensity_of_the_event += partial_sums2[e, x]
            log_likelihood += log(intensity_of_the_event)
        'Update the partial sums: impact of the new event'
        alpha = impact_coefficients[event, state]
        partial_sums[event, state] += alpha
        alpha2 = impact_coefficients2[event, state]
        partial_sums2[event, state] += alpha2
        previous_time = time
        'Compute the second term of the likelihood (l_{-})'
        time_increment = time_end - time
        beta = decay_coefficients[event, state]
...
def gradient(np.ndarray[DTYPEf_t, ndim=1] base_rates,
    ...
    cdef double alpha, beta, ratio, alpha2, beta2, ratio2, time_increment, time_increment_2, sample_duration, a, b, c, decay, a2, b2, c2, decay2, intensity_of_the_event
    ...
    cdef np.ndarray[DTYPEf_t, ndim = 3] gradient_impact_coefficients2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types))
    cdef np.ndarray[DTYPEf_t, ndim = 3] gradient_decay_coefficients2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types))
    ...
    '''Initialise the partial sums S_{e'x'e} and S^{(1)}_{e'x'e}, S_{2,e'x'e} and S^{(1)}_{2,e'x'e}
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 3] partial_sums2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
    cdef np.ndarray[DTYPEf_t, ndim= 3] partial_sums2_1 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
    ...
    for n in range(index_start):
        ...
        for e in range(number_of_event_types):
            ...
            beta2 = decay_coefficients2[event, state, e]
            ratio2 = impact_decay_ratios2[event, state, e]
            a2 = exp(- beta2 * time_increment)
            partial_sums2[event, state, e] += a2
            partial_sums2_1[event, state, e] += a2 * time_increment
            b2 = exp(- beta2 * time_increment_2)
            gradient_impact_coefficients2[event, state, e] -= (a2 - b2) / beta2
            gradient_decay_coefficients2[event, state, e] -= ratio2 * \
                (time_increment_2*b2 - time_increment*a2)
            gradient_decay_coefficients2[event,
                                        state, e] -= - ratio2 * (a2 - b2) / beta2
    ...
    for event in range(number_of_event_types):
        for state in range(number_of_states):
            for e in range(number_of_event_types):
                ...
                alpha2 = impact_coefficients2[event, state, e]
                partial_sums2[event, state,
                             e] = partial_sums2[event, state, e] * alpha2
                partial_sums2_1[event, state,
                               e] = partial_sums2_1[event, state, e] * alpha2
    ...
    for n in range(index_start, index_end):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                for e2 in range(number_of_event_types):
                    ...
                    beta2 = decay_coefficients2[e1, x, e2]
                    partial_sums2_1[e1, x, e2] += time_increment * \
                        partial_sums2[e1, x, e2]
                    decay2 = exp(-beta2 * time_increment)
                    partial_sums2_1[e1, x, e2] *= decay2
                    partial_sums2[e1, x, e2] *= decay2
        'Update the gradient'
        intensity_of_the_event = base_rates[event]
        for e in range(number_of_event_types):
            for x in range(number_of_states):
        ...
        for e in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                alpha2 = impact_coefficients2[e, x, event]
                gradient_impact_coefficients2[e, x, event] += (
                    partial_sums2[e, x, event] / alpha2) / intensity_of_the_event
                gradient_decay_coefficients2[e, x, event] -= partial_sums2_1[e,
                                                                           x, event] / intensity_of_the_event
        'Update the partial sums: impact of the new event'
        for e in range(number_of_event_types):
            alpha = impact_coefficients[event, state, e]
        ...
        for e in range(number_of_event_types):
            ...
            beta2 = decay_coefficients2[event, state, e]
            ratio2 = impact_decay_ratios2[event, state, e]
            c2 = 1 - exp(-beta2 * time_increment)
            gradient_impact_coefficients2[event, state, e] -= c2 / beta2
            gradient_decay_coefficients2[event, state,
                                        e] -= ratio2 * time_increment * (1 - c2)
            gradient_decay_coefficients2[event, state, e] -= - ratio2 * c2 / beta2
    'Return the result, i.e., the gradient'
    return gradient_base_rates, gradient_impact_coefficients, gradient_decay_coefficients, gradient_impact_coefficients2, gradient_decay_coefficients2 
...
def gradient_partial(int event_type,
    ...
    cdef int n, event, state, e, e1, x, e2, index_end
    cdef double time, previous_time
    cdef double alpha, beta, ratio, alpha2, beta2, ratio2, time_increment, time_increment_2, sample_duration, a, b, c, a2, b2, c2, decay, decay2, intensity_of_the_event
    # initialise the gradient vectors
    sample_duration = time_end - time_start 
    cdef double gradient_base_rate
    gradient_base_rate = - sample_duration # this is the partial derivative of l_{-} with respect to \nu_{k}
    ...
    cdef np.ndarray[DTYPEf_t, ndim = 2] gradient_impact_coefficients2 = np.zeros((number_of_event_types, number_of_states))
    cdef np.ndarray[DTYPEf_t, ndim = 2] gradient_decay_coefficients2 = np.zeros((number_of_event_types, number_of_states))
    ...
    for e1 in range(number_of_event_types):
        for x in range(number_of_states):
            alpha = impact_coefficients[e1, x]
            beta = decay_coefficients[e1, x]
            impact_decay_ratios[e1, x] = alpha / beta # store first set of impact/decay ratios
    ...
    '''Initialise the partial sums S_{e'x'} and S^{(1)}_{e'x'}, S_{2,e'x'} and S^{(1)}_{2,e'x'}
    that will allow us to compute the intensity and the gradient recursively;
    and compute contribution of initial condition on gradient.'''
    cdef np.ndarray[DTYPEf_t, ndim= 2] partial_sums = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf)
    cdef np.ndarray[DTYPEf_t, ndim= 2] partial_sums_1 = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf) # set up partial sums, initialise to 0
    
    cdef np.ndarray[DTYPEf_t, ndim= 2] partial_sums2 = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf)
    cdef np.ndarray[DTYPEf_t, ndim= 2] partial_sums2_1 = np.zeros((number_of_event_types, number_of_states), dtype=DTYPEf) # set up partial sums, initialise to 0
    ...
    for n in range(index_start):
        ...
        beta = decay_coefficients[event, state]
        ratio = impact_decay_ratios[event, state]
        a = exp(- beta * time_increment) # this is used in first term (of 3) of the partial derivative of l_{-} with respect to each \alpha
        partial_sums[event, state] += a # update partial sum
        partial_sums_1[event, state] += a * time_increment #update derivative of partial sum with respect to \beta
        b = exp(- beta * time_increment_2) # this is used in the second term (of 3) of the partial derivative of l_{-} with respect to each \alpha
        gradient_impact_coefficients[event, state] -= (a - b) / beta # this gives us the first and second terms in the partial derivative of l_{-} with respect to each \alpha - forming the portion of the derivative taken for times before the start t_{0}
        gradient_decay_coefficients[event,
                                    state] -= ratio * (time_increment_2*b - time_increment*a) #this gives us the first term (out of 4) for the partial derivative of l_{-} with respect to \beta
        gradient_decay_coefficients[event, state] -= - ratio * (a - b) / beta #this gives us the second term (out of 4) for the partial derivative of l_{-} with respect to \beta
    # By doing so, we multiply by the impact coefficients only once
        beta2 = decay_coefficients2[event, state]
        ratio2 = impact_decay_ratios2[event, state]
        a2 = exp(- beta2 * time_increment) # this is used in first term (of 3) of the partial derivative of l_{-} with respect to each \alpha
        partial_sums2[event, state] += a2 # update partial sum
        partial_sums2_1[event, state] += a2 * time_increment #update derivative of partial sum with respect to \beta
        b2 = exp(- beta2 * time_increment_2) # this is used in the second term (of 3) of the partial derivative of l_{-} with respect to each \alpha
        gradient_impact_coefficients2[event, state] -= (a2 - b2) / beta2 # this gives us the first and second terms in the partial derivative of l_{-} with respect to each \alpha - forming the portion of the derivative taken for times before the start t_{0}
        gradient_decay_coefficients2[event,
                                    state] -= ratio2 * (time_increment_2*b2 - time_increment*a2) #this gives us the first term (out of 4) for the partial derivative of l_{-} with respect to \beta
        gradient_decay_coefficients2[event, state] -= - ratio2 * (a2 - b2) / beta2 #this gives us the second term (out of 4) for the partial derivative of l_{-} with respect to \beta
    # By doing so, we multiply by the impact coefficients only once
    ...
    for event in range(number_of_event_types):
        for state in range(number_of_states):
            alpha = impact_coefficients[event, state] 
            partial_sums[event, state] = partial_sums[event, state] * alpha #update partial sums
            partial_sums_1[event, state] = partial_sums_1[event, state] * alpha #update first derivative of partial sums wrt \beta 
            
            alpha2 = impact_coefficients2[event, state] 
            partial_sums2[event, state] = partial_sums2[event, state] * alpha2 #update partial sums
            partial_sums2_1[event, state] = partial_sums2_1[event, state] * alpha2 #update first derivative of partial sums wrt \beta 
    ...
    for n in range(index_start, index_end):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                beta2 = decay_coefficients2[e1, x]
                partial_sums2_1[e1, x] += time_increment * partial_sums2[e1, x]
                decay2 = exp(-beta2 * time_increment)
                partial_sums2_1[e1, x] *= decay2
                partial_sums2[e1, x] *= decay2
        ...
        if event == event_type:
            ...
            for e in range(number_of_event_types):
                for x in range(number_of_states):
                    intensity_of_the_event += partial_sums[e, x] #we need the base rate and the partial sums to get the intensity
                    intensity_of_the_event += partial_sums2[e, x]
            gradient_base_rate += 1 / intensity_of_the_event # the partial derivative of l_{+} with respect to \nu
            for e in range(number_of_event_types):
                for x in range(number_of_states):
                    alpha = impact_coefficients[e, x]
                    if alpha > 0.:
                        gradient_impact_coefficients[e, x] += (
                            partial_sums[e, x] / alpha) / intensity_of_the_event #this is the partial derivative of l_{+} wrt to \alpha
                    gradient_decay_coefficients[e,
                                                x] -= partial_sums_1[e, x] / intensity_of_the_event # this is the partial derivative of l_{+} wrt \beta        'Update the partial sums: impact of the new event'
        
                    if alpha2 > 0.:
                        gradient_impact_coefficients2[e, x] += (
                            partial_sums2[e, x] / alpha2) / intensity_of_the_event #this is the partial derivative of l_{+} wrt to \alpha
                    gradient_decay_coefficients2[e,
                                                x] -= partial_sums2_1[e, x] / intensity_of_the_event # this is the partial derivative of l_{+} wrt \beta        'Update the partial sums: impact of the new event'
        ...
        ratio = impact_decay_ratios[event, state]
        c = 1 - exp(-beta * time_increment)
        gradient_impact_coefficients[event, state] -= c / beta # this is the third and final term in the partial derivative of l_{+} wrt \alpha
        gradient_decay_coefficients[event,
                                    state] -= ratio * time_increment * (1 - c) # third term out of 4 of the partial derivative of l_{-} wrt \beta
        gradient_decay_coefficients[event, state] -= - ratio * c / beta  # fourth and final term of the partial derivative of l_{-} wrt \beta
        ...
        ratio2 = impact_decay_ratios2[event, state]
        c2 = 1 - exp(-beta2 * time_increment)
        gradient_impact_coefficients2[event, state] -= c2 / beta2 # this is the third and final term in the partial derivative of l_{+} wrt \alpha
        gradient_decay_coefficients2[event,
                                    state] -= ratio2 * time_increment * (1 - c2) # third term out of 4 of the partial derivative of l_{-} wrt \beta
        gradient_decay_coefficients2[event, state] -= - ratio2 * c2 / beta2  # fourth and final term of the partial derivative of l_{-} wrt \beta

        
    'Return the result, i.e., the gradient'
    return gradient_base_rate, gradient_impact_coefficients, gradient_decay_coefficients, gradient_impact_coefficients2, gradient_decay_coefficients2
...
def simulate(int number_of_event_types,
             ...
             np.ndarray[DTYPEf_t, ndim=3] initial_partial_sums2,
             int initial_state,
             double time_start,
             double time_end,
    ...
    cdef double time, alpha, beta, alpha2, beta2
    ...
    for e2 in range(number_of_event_types):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                intensities[e2] += partial_sums2[e1, x, e2]
        ...
        if time <= time_end:  # if we are not out of the considered time window
            ...
            for e1 in range(number_of_event_types):
                for x in range(number_of_states):
                    for e2 in range(number_of_event_types):
                        ...
                        beta2 = decay_coefficients2[e1, x, e2]
                        partial_sums2[e1, x,
                                     e2] *= exp(-beta2 * random_exponential)
            ...
            if random_uniform < intensity_total:  # then yes, it is an event time
                ...
                for e in range(number_of_event_types):
                    ...
                    alpha2 = impact_coefficients2[event, state, e]
                    partial_sums2[event, state, e] += alpha2
                    intensities[e] += alpha2
                    intensity_total += alpha2
...
def compute_events_residuals(np.ndarray[DTYPEf_t, ndim=1] base_rates,
    ...
    cdef np.ndarray[DTYPEf_t, ndim= 3] partial_sums2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
    cdef np.ndarray[DTYPEf_t, ndim= 3] partial_sums2_old = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)
    cdef np.ndarray[DTYPEf_t, ndim= 3] impact_decay_ratios2 = np.zeros((number_of_event_types, number_of_states, number_of_event_types), dtype=DTYPEf)

    
    cdef int e1, x, e2, n, e, event, state, i, pos
    cdef double alpha, beta, alpha2, beta2, time, time_last
    ...
    '''Initialise the partial sums S_{e',x',e} S_{2,e',x',e} that will allow us to compute the residuals recursively.
    Note that, here, we work with (alpha_{e',x',e'}/beta_{e',x',e'})*S_{e',x',e'} instead of S_{e',x',e}, and likewise for the other partial sum'''
    for n in range(index_start):
        time = times[n]
        event = events[n]
        state = states[n]
        for e in range(number_of_event_types):
            ...
            beta2 = decay_coefficients2[event, state, e]
            partial_sums2[event, state, e] += exp(-beta2 * (time_start - time))
    ...
    for event in range(number_of_event_types):
        for state in range(number_of_states):
            for e in range(number_of_event_types):
                ...
                partial_sums2[event, state, e] = partial_sums2[event,
                                                             state, e] * impact_decay_ratios2[event, state, e]
    ...
    for e1 in range(number_of_event_types):
        for x in range(number_of_states):
            for e2 in range(number_of_event_types):
                ...
                partial_sums2[e1, x, e2] += initial_partial_sums2[e1, x, e2]
                partial_sums2_old[e1, x, e2] = partial_sums2[e1, x, e2]
    ...
    for n in range(index_start, length):
        ...
        for e1 in range(number_of_event_types):
            for x in range(number_of_states):
                for e2 in range(number_of_event_types):
                    ...
                    partial_sums2[e1, x,
                                 e2] *= exp(-decay_coefficients2[e1, x, e2] * (time - time_last))
        ...
        for e in range(number_of_event_types):
            for x in range(number_of_states):
                ...
                residuals[event, pos] += partial_sums2_old[e,
                                                          x, event] - partial_sums2[e, x, event]
                # save new partial sums to compute the next residual for this event type
                partial_sums2_old[e, x, event] = partial_sums2[e, x, event]
        ...
        for e in range(number_of_event_types):
            ...
            partial_sums2[event, state,
                         e] += impact_decay_ratios2[event, state, e]
...
def compute_total_residuals(np.ndarray[DTYPEf_t, ndim=3] transition_probabilities,
    ...
    cdef double alpha, beta, alpha2, beta2, time, time_last, phi
    'Compute ratios alpha/beta just once'
    for e1 in range(number_of_event_types):
        for x in range(number_of_states):
            for e2 in range(number_of_event_types):
    ...
    '''Initialise the partial sums S_{e',x',e}, S_{2,e',x',e} that will allow us to compute the residuals recursively.
    Note that, here, we work with (alpha_{e',x',e'}/beta_{e',x',e'})*S_{e',x',e'} instead of S_{e',x',e}, and likewise for S_{2,e',x',e}'''
    for n in range(index_start):
        time = times[n]
        event = events[n]
        state = states[n]
            ...
            for x in range(number_of_states):
                ...
                for e2 in range(number_of_event_types):
                    for x2 in range(number_of_states):
                        ...
                        residuals[e, x, pos] += phi * \
                            (partial_sums2_old[e2, x2, e] -
                             partial_sums2[e2, x2, e])
                ...
                if e != event:
                    ...
                    residuals[e, x,
                              pos] += impact_decay_ratios2[event, state, e]*phi
                if e == event:  # in this case, this event contributes to the residual betwen time and next_time; hence the new current state must be used
                    ...
                    if x == state:
                        ...
                        residuals[e, x, pos +
                                  1] += impact_decay_ratios2[event, state, e]*phi
